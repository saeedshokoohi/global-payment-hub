...
import com.eyeson.payment_hub.config.KafkaProperties;
...
class GlobalPaymentHubKafkaResourceIT {
    ...
    void setup() {
        ...
        GlobalPaymentHubKafkaResource kafkaResource = new GlobalPaymentHubKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/global-payment-hub-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/global-payment-hub-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
